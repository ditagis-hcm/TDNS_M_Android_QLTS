...
import hcm.ditagis.com.tdns.qlts.adapter.FeatureViewMoreInfoAdapter;
import hcm.ditagis.com.tdns.qlts.adapter.ObjectsAdapter;
import hcm.ditagis.com.tdns.qlts.adapter.SearchAdapter;
import hcm.ditagis.com.tdns.qlts.async.UpdateAttachmentAsync;
import hcm.ditagis.com.tdns.qlts.entities.entitiesDB.LayerInfoDTG;
import hcm.ditagis.com.tdns.qlts.entities.entitiesDB.ListObjectDB;
import hcm.ditagis.com.tdns.qlts.libs.Action;
import hcm.ditagis.com.tdns.qlts.libs.FeatureLayerDTG;
import hcm.ditagis.com.tdns.qlts.tools.ThongKe;
import hcm.ditagis.com.tdns.qlts.utities.Config;
import hcm.ditagis.com.tdns.qlts.utities.ImageFile;
import hcm.ditagis.com.tdns.qlts.utities.ListConfig;
import hcm.ditagis.com.tdns.qlts.utities.MapViewHandler;
import hcm.ditagis.com.tdns.qlts.tools.MySnackBar;
import hcm.ditagis.com.tdns.qlts.utities.Popup;
import hcm.ditagis.com.tdns.qlts.tools.SearchItem;
...
public class QuanLyTaiSan extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener, View.OnClickListener {
    ...
    private void initMapView() {
        mMap = new ArcGISMap(Basemap.Type.OPEN_STREET_MAP, 10.7554041, 106.6546293, 10);
...
//            preparingAsycn.execute();
    ...
    private void setFeatureService2(){
    ...
    private void setFeatureService() {
        ...
        for (final Config config : configs) {
            if (config.getGroupLayer().equals(getString(R.string.GROUP_BASE_MAP))) {
                hanhChinhImageLayers = new ArcGISMapImageLayer(config.getUrl());
                hanhChinhImageLayers.setId(config.getLayerID());
            ...
            if (config.getGroupLayer().equals(getString(R.string.GROUP_TAI_SAN))) {
                ...
                featureLayer.setId(config.getLayerID());
                mMap.getOperationalLayers().add(featureLayer);
//                featureLayer.setMinScale(config.getMinScale());
                FeatureLayerDTG featureLayerDTG = new FeatureLayerDTG(featureLayer);
                Action action = new Action(true,true,true,true);
                featureLayerDTG.setAction(action);
                featureLayerDTG.setOutFields(config.getUpdateField());
                featureLayerDTG.setQueryFields(config.getUpdateField());
...
//                    edit_latitude.setText(location[1] + "");
    ...
    private void setFeatureService1() {
        ...
        for (final LayerInfoDTG layerInfoDTG : ListObjectDB.getInstance().getLstFeatureLayerDTG()) {
            ...
            if (layerInfoDTG.getId().equals(getString(R.string.GROUP_BASE_MAP))) {
